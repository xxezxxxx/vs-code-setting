...
const resolveHighlightNote = (
  ...
  if (typeof input === "object" && !Array.isArray(input)) {
    const noteObject = input as { title?: unknown; body?: unknown; icon?: unknown }
...
function MyComponent({ args, disabled, theme }: ComponentProps): ReactElement {
  ...
  const highlightMatch = useMemo(() => {
    if (highlightTargetColumn === null || highlightReferenceTimestamp === null) {
      return null
    }
    ...
    return { rowIndex: closestIndex, column: highlightTargetColumn, diff: closestDiff }
  ...
  const resolvedHighlightNote = useMemo(() => {
    ...
    const diffText = formatDifference(targetTimestamp, highlightReferenceTimestamp)
  ...
  }, [highlightMatch, highlightReferenceTimestamp, highlightRowTimestamp, highlightNoteRaw])
  ...
  }, [highlightNoteCardBackground, highlightOutlineColor, accentShadowColor])
                  ...
                  <tbody>
                    {rows.map((row, rowIndex) => {
                      const isHighlightRow =
                        highlightMatch !== null && highlightMatch.rowIndex === rowIndex
                      ...
                      return (
                        <tr key={rowIndex} style={rowInlineStyle}>
                          {columns.map((column) => {
                            ...
                            return (
                              <td key={`${column}-${rowIndex}`} style={cellInlineStyle}>
                                {renderLinkButton ? (
                                  <a
            ...
            {shouldShowHighlightNote ? (
              <div style={highlightNoteCardStyle}>
                <div style={highlightNoteTitleStyle}>
                  ...
                  <span>{resolvedHighlightNote?.title ?? highlightNoteHeading}</span>
                </div>
                <div style={highlightNoteBodyStyle}>
                  {resolvedHighlightNote?.body ?? "ref_ts와 가장 가까운 행입니다."}
                </div>
              </div>
            ) : null}
          </div>
