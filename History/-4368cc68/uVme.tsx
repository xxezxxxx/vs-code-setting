...
type AlarmNote = { text: string; level?: "info" | "warn" | "error" } | string | null
...
type Args = {
  ...
  shortlog_jump_buttons?: Record<string, Record<string, string | number | (string | number)[]>> | null
}
...
const styles = (accent: string, maxWidth?: number | string | null) => ({
  ...
  // Alert/Note styles (no box; text only)
  alertInfo: {
    display: "block",
    padding: 0,
...
function ShortLogView(props: {
  ...
  jumpButtons?: Record<string, Record<string, string | number | (string | number)[]>> | null
  ...
  return (
    <div>
      ...
      {(() => {
        ...
        const note = typeof alarmNote === "string" ? { text: alarmNote, level: "info" as const } : alarmNote
        if (!note || !note.text) return null
        const lv = (note.level || "info").toLowerCase()
        const style = lv === "error" ? s.alertError : lv === "warn" ? s.alertWarn : s.alertInfo
          ...
          >
            {filtered.map((r, i) => {
              ...
              for (const rule of effRules) {
                ...
                let val = ""
                if (colLowerName === "rowindex" || colLowerName === "index" || colLowerName === "__index") {
                  val = String(rows.indexOf(r))
                } else if (colLowerName === "filteredindex" || colLowerName === "__filteredindex") {
                  val = String(i)
                } else {
                ...
                if (
                  rule.equals &&
                  rule.equals.some((x) => String(x).toLowerCase() === val.toLowerCase())
                )
                  matched = true
                else if (
                  rule.includes &&
        ...
        <div style={s.itemWrap}>
          ...
          <div style={s.panel}>
            {(() => {
              ...
              if (!note) return null
              const n = typeof note === "string" ? { text: note, level: "info" as const } : note
