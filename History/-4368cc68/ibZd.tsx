...
function ShortLogView(props: {
  ...
  jumpButtons?: Record<string, Record<string, string | string[]>> | null
}) {
  const {
    data,
    filterConfig,
  ...
  // Open matching rows in a new window (plain HTML table)
  const openViewAllWindow = (label: string) => {
    const idxs = matchesByLabel.get(label) || []
    if (!idxs.length) return
    const rows = idxs.map((i) => filtered[i])
    const cols = (layoutOrder && layoutOrder.length > 0) ? layoutOrder : columns
    const w = window.open("", "_blank")
    if (!w) return
    const esc = (s: any) => String(s ?? "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;")
    const head = `<!doctype html><html><head><meta charset="utf-8" />
    <title>${esc(label)} – ${rows.length} rows</title>
    <style>
      body { font-family: Inter, system-ui, -apple-system, Segoe UI, Roboto, Arial, sans-serif; margin: 16px; color:#111; }
      h3 { margin: 0 0 12px; font-size:16px; }
      table { border-collapse: collapse; width: 100%; }
      th, td { border: 1px solid #e0e0e0; padding: 6px 8px; font-size: 12px; white-space: nowrap; }
      th { background: #f7f7f7; text-align: left; }
      tr:nth-child(even) td { background: #fafafa; }
    </style></head><body>`
    const header = `<h3>${esc(label)} – ${rows.length} rows</h3>`
    const thead = `<thead><tr>${cols.map((c) => `<th>${esc(c)}</th>`).join("")}</tr></thead>`
    const tbody = `<tbody>${rows.map((r) => `<tr>${cols.map((c) => `<td>${esc((r as any)[c])}</td>`).join("")}</tr>`).join("")}</tbody>`
    const html = `${head}${header}<table>${thead}${tbody}</table></body></html>`
    w.document.open()
  ...
  const matchesByLabel = useMemo(() => {
    ...
    const matchRules = (row: any, rules: Record<string, string | string[]>) => {
      for (const [col, termOrList] of Object.entries(rules || {})) {
        const val = norm((row as any)[col])
        if (Array.isArray(termOrList)) {
          const ok = termOrList.some((t) => val.includes(norm(t)))
          if (!ok) return false
        } else {
          if (!val.includes(norm(termOrList))) return false
        }
      }
      return true
    }
    for (const { label, rules } of jumpDefs) {
      ...
      for (let i = 0; i < filtered.length; i++) {
        const r = filtered[i]
        if (matchRules(r, rules)) idxs.push(i)
      }
      m.set(label, idxs)
    }
    return m
  ...
  const viewAllRows = useMemo(() => {
    if (!viewAllLabel) return [] as any[]
    const idxs = matchesByLabel.get(viewAllLabel) || []
    return idxs.map((i) => filtered[i])
  }, [viewAllLabel, matchesByLabel, filtered])
              ...
              <div key={label} style={s.splitWrap}>
                ...
                >
                  ...
                  >
                    ...
                    <button
                      style={s.menuItem}
                      onClick={() => { setOpenJumpMenu(null); openViewAllWindow(label) }}
                    >
                      전체 보기(새 창)
                    </button>
                      전체 보기
          ...
          >
            {filtered.map((r, i) => {
              ...
              for (const rule of effRules) {
                ...
                if (
                  rule.equals &&
                  rule.equals.some((x) => x.toLowerCase() === val.toLowerCase())
                )
                  matched = true
                else if (
                  rule.includes &&
            ...
            <div style={s.modalBody}>
              ...
              >
                {viewAllRows.map((r, i) => {
                  const display = stringifyRow(r, columns)
                  return (
                    <div key={i} style={s.logRow} title={display}>
                      {(
                        layoutOrder
