...
function normalizeAlarmNotes(
  ...
  return input
    ...
    .map((note) => {
      const level = (note.level || "info").toLowerCase() as "info" | "warn" | "error"
      const style =
        level === "error"
...
function MyComponent({ args, theme }: ComponentProps): ReactElement {
  ...
  const s = useMemo(() => createStyles(accent, a.max_width ?? null), [accent, a.max_width])
  ...
  const shortLength = (a.error_log_short?.records?.length as number | undefined) || 0
  useEffect(() => {
  ...
  const reportCount = (a.report_list?.records?.length as number | undefined) || 0
  const serverIndex =
    typeof a.active_report_index === "number" && !Number.isNaN(a.active_report_index)
  ...
  const reportHtml: string | null | undefined = usingCache ? cacheHtml : a.report_detail_html

  const detailRow = selectedIndex != null ? detailData?.records?.[selectedIndex] : null
  ...
  const buildFields = useCallback(
    (row: any) => {
      ...
      const idKey = ["id", "uuid", "_id", "report_id", "index"].find((key) => key in (row || {}))
  ...
  const { ref: listWrapRef, style: listCollapseStyle } = useCollapsible(!reportListCollapsed, 280)

  const notes = useMemo(() => normalizeAlarmNotes(a.report_detail_alarm_note, s), [a.report_detail_alarm_note, s])
        ...
        <div style={s.itemWrap}>
          <div style={s.headerRow}>
            ...
            <button
              ...
              aria-label={reportListCollapsed ? "Expand report list" : "Collapse report list"}
          ...
          <div style={s.panel}>
            {reportListCollapsed ? (
              <CompactSelectedReport
                record={(activeIndex != null ? a.report_list?.records?.[activeIndex] : undefined) as any}
        ...
        <div style={s.itemWrap}>
            ...
            <div style={{ position: "relative", display: "inline-block" }}>
              <button
                ...
                onClick={async () => {
                  ...
                  const text = rows && rows.length ? stringifyDetail(rows[0], cols) : ""
                  if (text) {
                    try {
          ...
          <div style={s.panel}>
            <DetailLogPanel
              ...
              showHeader={false}
              maxHeight={a.detail_max_height ?? (reportListCollapsed ? 420 : 300)}
              minHeight={a.detail_min_height ?? (reportListCollapsed ? 420 : 300)}
            />
          </div>
        </div>
