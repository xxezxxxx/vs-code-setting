...
function ShortLogView(props: {
  ...
  const centerNextRef = React.useRef(false)   // ✅ 다음 선택 시 센터 스크롤 여부
  ...
  if (!centerNextRef.current) return         // ✅ 자동 선택이 아닐 땐 아무 것도 안 함
  ...
  requestAnimationFrame(() => {
    ...
    cont.scrollTo({ top: nextTop, behavior: "auto" }) // “딱딱” 이동(원하면 "smooth")

    centerNextRef.current = false  // ✅ 1회성 플래그 해제
  })
}, [active, filtered])
  ...
  return (
    <div>
      ...
      {(() => {
        ...
        // string -> {text, level:"info"}, null 필터링, text 없는 것도 제외
...
function DetailLogPanel(props: {
  ...
  return (
    <div>
      {showHeader && (
        <div style={{ ...s.header, display: "flex", justifyContent: "space-between" }}>
...
function MyComponent({ args, theme }: ComponentProps): ReactElement {
  ...
  console.log("detail_min_height", a.detail_min_height)
  console.log("detail_max_height", a.detail_max_height)
  return (
    <div style={s.container}>
      ...
      >
        ...
        <div style={s.itemWrap}>
          ...
          <div style={s.panel}>
            <DetailLogPanel
              ...
              minHeight={
                 a.detail_min_height ?? (reportListCollapsed ? 200 : 150)
              }       
              onCopy={() => {
        ...
        <div style={s.itemWrap}>
          ...
          <div style={s.panel}>
          {(() => {
            const raw = a.report_detail_alarm_note as
              | ({ text: string; level?: "info" | "warn" | "error" } | string | null)
              | Array<{ text: string; level?: "info" | "warn" | "error" } | string | null>
              | undefined;

            const normalized = raw == null ? [] : Array.isArray(raw) ? raw : [raw];

            const notes = normalized
              .map((n) => (typeof n === "string" ? { text: n, level: "info" as const } : n))
              .filter(
                (n): n is { text: string; level?: "info" | "warn" | "error" } => !!n && !!n.text
              );
            ...
            return (
              <div style={s.notesWrap}>
                {notes.map((n, i) => {
                  ...
                  const style =
                    lv === "error"
                      ? { ...s.noteBadgeBase, ...s.noteBadgeError }
                      : lv === "warn"
                      ? { ...s.noteBadgeBase, ...s.noteBadgeWarn }
                      : { ...s.noteBadgeBase, ...s.noteBadgeInfo };
                  return (
                    <span key={`report-note-${i}`} style={style}>
                      {lv === "error" ? (
                        <IconError size={14} />
                      ) : lv === "warn" ? (
                        <IconWarn size={14} />
