...
    `;
    ...
    const option = {
      tooltip: { trigger: "axis" }, // 툴박스를 사용하여 확대/축소, 되돌리기 버튼만 표시
      toolbox: {
        show: true,
        orient: "horizontal", // 툴박스 위치를 상단으로 변경 (보기 쉽게)
        right: 10,
        top: 0,
        feature: {
          // **영역 확대/축소 버튼 활성화**
          dataZoom: {
            // 버튼 클릭 시 영역 선택 모드로 전환
            title: { zoom: "영역확대", back: "되돌리기" },
            yAxisIndex: false, // Y축 드래그 확대 비활성화
          }, // **초기 상태로 되돌리기 버튼 활성화** (restore)
          restore: {
            title: "초기화",
          }, // 다른 버튼(저장, 데이터 뷰 등)은 모두 제거
        },
      },
      legend: { data: datasetsForThisChart.map((d) => d.source) },
      grid: {
        left: "3%",
        right: "10%", // 툴박스 위치 확보를 위해 조정
        bottom: "15%",
        top: "20%",
        containLabel: true,
      },
      ...
      yAxis: { type: "value" },
      series: seriesData, // dataZoom 컴포넌트 (내부): 휠 확대만 허용, 드래그 Pan 기능은 비활성화
      dataZoom: [
        {
          type: "inside",
          xAxisIndex: 0,
          filterMode: "none", // **휠 확대/축소만 허용**
          zoomOnMouseWheel: true, // 드래그 Pan (이동) 기능 비활성화
          moveOnMouseMove: false, // 마우스 드래그로 박스 선택 기능 비활성화 (툴박스 버튼이 담당)
    ...
    document.addEventListener(
      ...
      (e) => {
        ...
        isDragging = false; // 1. 이동 거리가 임계값 미만이면 무시
        ...
        if (dx < 0) {
          ...
          resetZoom(divId);
        } // 3. 정방향 드래그 (dx > 0)는 툴박스에서 '영역확대' 버튼을 누른 상태라면 Box Zoom이 담당합니다.
      },
      { once: true }
    );
  });
