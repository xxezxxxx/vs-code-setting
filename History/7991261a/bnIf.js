    ...
    const onZoomComplete = ({ chart }) => {
      ...
      if (resetBtn) {
        if (isZoomed) {
          ...
          resetBtn.textContent = "✅ 확대됨: 🔄 Zoom 초기화 (드래그 시 초기화)";
        } else {
          resetBtn.style.opacity = "0.5";
          resetBtn.style.pointerEvents = "none";
          resetBtn.textContent = "🔄 Zoom 초기화 (드래그 시 초기화)";
        }
      }
    };
        ...
        options: {
          ...
          plugins: {
            ...
            zoom: {
              zoom: {
                drag: {
                  enabled: false, // [!!! 최종 수정: 드래그 Zoom 기능 비활성화 !!!]
              ...
              pan: {
                ...
                mode: "xy",
                modifierKey: "shift",
              }, // onZoomStart와 dragStartX, dragStartY는 이제 필요 없지만 플러그인의 이벤트 로직을 위해 남겨둡니다.

              onZoomStart({ chart, event }) {
                chart.dragStartX = event.x;
                chart.dragStartY = event.y;
              }, // beforeZoom 로직: 드래그가 아닌 휠 확대 등이 발생했을 때 초기화 기능을 수행합니다.

              beforeZoom({ chart, event }) {
                const isCurrentlyZoomed = chart.isZoomedOrPanned(); // 만약 휠 줌 등으로 확대된 상태에서 또다시 Zoom 이벤트가 발생하면 초기화합니다.

                // (drag:false 이므로 이 이벤트는 마우스 드래그에 의해 발생할 가능성은 낮습니다.)
                ...
                // 차트가 초기 상태라면, 정상적으로 확대합니다. (휠 줌은 허용)
                return true;
              },
