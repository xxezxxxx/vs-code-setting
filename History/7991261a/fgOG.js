...
/**
 * 모든 차트와 메시지를 초기화합니다. (사용자 제공 코드 유지)
 */
...
/**
 * 특정 차트의 Zoom을 초기화하고, X축 전용 Box Zoom 모드로 재진입시킵니다.
...
const resetZoom = (divId) => {
  ...
  if (chart) {
    ...
    }); // 2. 초기화 후, X축 전용 Box Zoom 모드로 재진입

    chart.dispatchAction({
      type: "takeGlobalCursor",
      key: "dataZoom",
      dataZoomOption: {
        xAxisIndex: [0], // **[수정] X축 Zoom 활성화 유지**
        yAxisIndex: [], // **[수정] Y축 Zoom 비활성화**
      },
    });
  }
};
...
    `;
    ...
    const option = {
      ...
      toolbox: {
        ...
        feature: {
          dataZoom: {
            title: { zoom: "영역확대", back: "되돌리기" }, // **[수정] X축만 Box Zoom의 대상이 되도록 명시**
            xAxisIndex: [0],
            yAxisIndex: false, // Y축 확대 비활성화
          },
          restore: {
            title: "초기화",
          },
      ...
      dataZoom: [
        {
          type: "inside",
          xAxisIndex: 0, // **[수정] 휠 확대도 X축만 적용**
          yAxisIndex: false,
          filterMode: "none",
          zoomOnMouseWheel: true, // 휠은 X축 Zoom // 드래그는 Box Zoom이 담당 (Pan 비활성화)
    ...
    chartMap.set(divId, myChart); // **[핵심 추가] 차트 로드 직후, 드래그가 바로 X축 Box Zoom 모드가 되도록 강제 설정**
...
// ===============================================
// 2, 3, 5, 4, 6번 함수는 변경 사항 없음 (사용자 제공 코드 유지)
// ===============================================
