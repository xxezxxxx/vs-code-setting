...
/**
 * 특정 차트의 Zoom/Pan을 초기화합니다. (버튼 제거로 인해 내부 호출용으로 변경)
...
    `; // 버튼 제거
    ...
    const option = {
      ...
      yAxis: { type: "value" },
      series: seriesData, // dataZoom 컴포넌트: Pan/Zoom 기능을 활성화합니다.
      dataZoom: [
        {
          type: "inside",
          xAxisIndex: 0,
          filterMode: "none",
          // **핵심 수정: 마우스 휠 Zoom/드래그 Pan 활성화**
    ...
    const DRAG_THRESHOLD = 5; // 마우스 다운 (드래그 시작) - 차트 DOM에 등록
    ...
    // 주의: ECharts의 Pan 기능은 정방향 드래그를 내부적으로 처리합니다.
    // 여기서는 역방향 드래그 시 초기화만 수행하도록 합니다.
    document.addEventListener(
      ...
      (e) => {
        ...
        // 3. 정방향 드래그 (dx > 0)는 ECharts의 기본 Pan 기능이 담당합니다.
      },
      { once: true }
    ); // 마우스가 차트 영역을 벗어날 때

    chartDom.addEventListener("mouseleave", () => {
      // mouseleave 시 상태를 끄지 않아 document의 mouseup이 최종 처리를 하도록 유지
    });
  });
};
