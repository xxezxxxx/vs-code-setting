...
    `;
          ...
          plugins: {
            ...
            zoom: {
              ...
              pan: {
                ...
                mode: "xy",
                modifierKey: "shift",
              }, // 드래그 시작 지점 기억 (이전 로직은 이제 사용되지 않지만 안전을 위해 남겨둡니다.)

              onZoomStart({ chart, event }) {
                chart.dragStartX = event.x;
                chart.dragStartY = event.y;
              }, // ⬇⬇⬇ [수정됨] 드래그 시 확대 상태라면 무조건 초기화하고 Zoom 차단 ⬇⬇⬇

              beforeZoom({ chart, event }) {
                // 현재 차트가 Zoom 또는 Pan 상태인지 확인
                const isCurrentlyZoomed = chart.isZoomedOrPanned(); // 차트가 확대된 상태라면, 드래그 방향에 상관없이 초기화합니다.

                if (isCurrentlyZoomed) {
                  chart.resetZoom(); // 버튼 상태 수동 업데이트
    ...
    chartMap.set(canvasId, newChart); // 초기상태 버튼 설정
