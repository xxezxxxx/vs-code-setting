...
    `;
    ...
    const option = {
      tooltip: { trigger: "axis" }, // 툴박스를 사용하여 확대/축소/이동/초기화 버튼만 표시
      toolbox: {
        show: true,
        orient: "horizontal",
        right: 10,
        ...
        feature: {
          ...
          brush: {
            type: ["move"],
            title: { move: "이동" },
          }, // 2. 영역 확대/축소 버튼 활성화 (XY축 모두)
          dataZoom: {
            title: { zoom: "영역확대", back: "되돌리기" },
            yAxisIndex: [0], // Y축 확대 허용 (true 대신 0번 축 지정)
          }, // 3. 초기 상태로 되돌리기 버튼 활성화
          restore: {
            title: "초기화",
      ...
      yAxis: { type: "value" },
      series: seriesData, // dataZoom 컴포넌트 (내부): 휠 확대와 드래그 이동 활성화
      dataZoom: [
        {
          type: "inside",
          xAxisIndex: 0,
          yAxisIndex: 0, // Y축 확대/축소 허용
          filterMode: "none", // **휠 확대/축소 활성화 (X, Y축 모두)**
          zoomOnMouseWheel: true, // **드래그 Pan (이동) 기능 활성화**
          moveOnMouseMove: true, // 마우스 드래그로 박스 선택 기능 비활성화 (툴박스 버튼이 담당)
