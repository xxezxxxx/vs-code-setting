...
export const api = {
  ...
  async sum(a: string | number, b: string | number): Promise<number> {
  ...
  async setmypage(a: string, b: string): Promise<string> {
    const res = await fetch(`${BASE}/api/mypage`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ message: `${a}:${b}` })
    })
    const data = await json<{ message: string }>(res)
    return data.message
  },

  async multiply(a: string | number, b: string | number): Promise<number> {
