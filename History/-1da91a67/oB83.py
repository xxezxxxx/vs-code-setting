...
def sub_menu(
    *,
    ...
    required: Optional[List[str]] = None,      # 해제 불가 항목(체크박스 disabled)
    ...
    Parameters
    ----------
    - order       : 렌더링할 전체 항목 + 초기 순서
    - checked     : 미리 체크된 항목
    - required    : 체크 해제 불가 항목 (체크박스 disabled)
    - options     : UI에서 렌더하지 않음, save/close 시 그대로 반환
    - list_height : 내부 스크롤 영역 높이(px)
    - frame_height: iframe 고정 높이(px)
    - ack         : {"save_ok": True} 전달 시 저장 성공 토스트
...
if __name__ == "__main__":
    ...
    st.title("sub_menu Demo (order/checked only + required + options passthrough)")
    ...
    result = sub_menu(
        ...
        required=["col_2", "col_7"],   # 해제 불가(디폴트 체크 상태 유지)
        options={"limit": "1000", "mode": "fast"},  # 렌더 X, 그대로 반환
    ...
    if result:
        ...
        if result["event"] == "save":
            # 실제 저장 로직 처리 후, 다음 렌더에 ack로 성공 토스트
