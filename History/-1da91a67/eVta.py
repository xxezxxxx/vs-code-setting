...
if __name__ == "__main__":
    ...
    if not "initial" in st.session_state:
    ...
    with col1:
        st.multiselect("hi", ["a","b","c"])
    with col2:
        btn_setting = st.button("setting")
        
    
    if btn_setting:
        st.session_state.set_num += 1
    
    st.write(st.session_state.set_num)

    if st.session_state.set_num % 2 == 0:
        result = sub_menu(
