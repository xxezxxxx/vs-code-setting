    ...
    <div style={{ ...widthStyle }}>
        ...
        >
          ...
          {nav_buttons && (
            <div style={{ display: "inline-flex", gap: 8 }}>
              <button
                type="button"
                onClick={() => goToNext(nav_terms?.error || "ERROR")}
                disabled={disabled}
                style={{
                  padding: "6px 10px",
                  borderRadius: 8,
              ...
              <button
                type="button"
                onClick={() => goToNext(nav_terms?.warn || "WARN")}
                disabled={disabled}
                style={{
                  padding: "6px 10px",
                  borderRadius: 8,
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {/* (moved to global toolbar above titles) */}
          <div
            style={{
              display: "none",
            }}
...
const goToNext = useCallback(
  (term: string) => {
    ...
    if (!t) return
    const col = nav_column
    const hits: number[] = []
    visibleShortRows.forEach((row, vIdx) => {
      const val = toStr(row?.[col])
      if (val.toLowerCase().includes(t)) hits.push(vIdx)
    })
    if (hits.length === 0) return
    const last = lastPosRef.current[t] ?? -1
    // Start from current selection if present
    let startFrom = selectedVisibleIndex ?? -1
    const next = hits.find((v) => v > Math.max(last, startFrom)) ?? hits[0]
    lastPosRef.current[t] = next
    setSelectedOriginalIndex(filteredIndexMap[next])
  },
  [visibleShortRows, filteredIndexMap, selectedVisibleIndex, nav_column]
)
