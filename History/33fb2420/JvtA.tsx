...
function TableView({
  ...
  // Auto-hide scrollbar: show only while actively scrolling/dragging, hide after 1s
  const [showScroll, setShowScroll] = useState(false)
  const hideTimerRef = useRef<number | null>(null)
  const triggerShowScroll = useCallback(() => {
    setShowScroll(true)
    if (hideTimerRef.current) window.clearTimeout(hideTimerRef.current)
    hideTimerRef.current = window.setTimeout(() => setShowScroll(false), 1000)
  }, [])
  useEffect(() => {
    return () => {
      if (hideTimerRef.current) window.clearTimeout(hideTimerRef.current)
  ...
  // Custom overlay scrollbar (native hidden)
  const scrollRef = useRef<HTMLDivElement>(null)
  const [thumbTop, setThumbTop] = useState(0)
  const [thumbH, setThumbH] = useState(24)
  const recalcThumb = useCallback(() => {
    ...
    const ratio = ch / sh
    const h = Math.max(28, Math.floor(ch * ratio))
    const track = ch - h
    const top = Math.floor((st / (sh - ch)) * track)
    setThumbH(h)
    setThumbTop(top)
  }, [])
  const onScrollAll = useCallback(() => {
    recalcThumb()
    triggerShowScroll()
  }, [recalcThumb, triggerShowScroll])
  useEffect(() => {
    recalcThumb()
  }, [rows, columns, maxHeight, recalcThumb])
  ...
  const onThumbMouseDown = (e: React.MouseEvent) => {
    ...
    const onMove = (ev: MouseEvent) => {
      ...
      let newTop = dragRef.current.startTop + delta
      newTop = Math.max(0, Math.min(track, newTop))
      const scrollTop = (newTop / track) * (sh - ch)
  ...
  return (
    ...
    >
      {/* Data area only scrollbar (auto-hide after 1s; show only while scrolling; square thumb; no arrows/track/corner) */}
      <style>
        {`
          /* Firefox */
          div[data-role="table-scroll"]{ scrollbar-width: none; }
          div[data-role="table-scroll"][data-show="1"]{ scrollbar-width: thin; }
          /* WebKit */
          /* hide gutter when not scrolling */
          div[data-role="table-scroll"]::-webkit-scrollbar{ width:0px; height:0px; }
          div[data-role="table-scroll"][data-show="1"]::-webkit-scrollbar{ width:8px; height:8px; }
          /* remove arrows/buttons entirely */
          div[data-role="table-scroll"]::-webkit-scrollbar-button{ display:none; width:0; height:0; }
          div[data-role="table-scroll"]::-webkit-scrollbar-button:single-button{ display:none; width:0; height:0; }
          /* transparent track and corner so only the thumb is visible */
          div[data-role="table-scroll"]::-webkit-scrollbar-track{ background: transparent !important; }
          div[data-role="table-scroll"]::-webkit-scrollbar-track-piece{ background: transparent !important; }
          div[data-role="table-scroll"]::-webkit-scrollbar-corner{ background: transparent !important; }
          div[data-role="table-scroll"]::-webkit-scrollbar-thumb{ background: transparent; border-radius:0; }
        `}
      </style>
...
function MyComponent({ args, disabled }: ComponentProps): ReactElement {
  ...
  useEffect(() => {
    // Fix the outer frame height to the provided height so inner areas scroll
