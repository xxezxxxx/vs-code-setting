...
function MyComponent({ args, disabled }: ComponentProps): ReactElement {
  ...
  const goToNext = useCallback(
    (spec: { columns: string[]; terms: string[]; key: string }) => {
      ...
      const terms = (spec.terms || []).map((s) => (s || "").toLowerCase()).filter(Boolean)
          ...
          <div style={{ display: "inline-flex", gap: 8 }}>
            {(() => {
              ...
              if ((args as any).buttons) {
                const btns = (args as any).buttons as Record<string, Record<string, string[]>>
                ...
                items.push(
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      goToNext({ columns: [nav_column], terms: [nav_terms?.error || "ERROR"], key: "__error" })
                    }
                    disabled={disabled}
                    style={{
                      padding: "6px 10px",
                ...
                items.push(
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      goToNext({ columns: [nav_column], terms: [nav_terms?.warn || "WARN"], key: "__warn" })
                    }
                    disabled={disabled}
                    style={{
                      padding: "6px 10px",
