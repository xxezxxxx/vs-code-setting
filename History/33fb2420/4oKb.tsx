...
function TableView({
  ...
  return (
    ...
    >
      {/* lightweight custom scrollbar for data area only */}
      <style>
        {`
          div[data-role="table-scroll"]{ scrollbar-width: thin; }
          div[data-role="table-scroll"]::-webkit-scrollbar{ width:8px; height:8px; }
          div[data-role="table-scroll"]::-webkit-scrollbar-thumb{ background:${hexToRgba(accent,0.3)}; border-radius:8px; }
      ...
      {/* Body table (scrolls) */}
      <div data-role="table-scroll" style={{ maxHeight, overflowY: "auto", overflowX: "hidden" }}>
        <table
          style={{
            width: "100%",
            borderCollapse: "separate",
              ...
              rows.map((row, ri) => {
                ...
                return (
                  ...
                  >
                    {columns.map((col) => {
                      const val = row[col]
                      const isLevel = ["level", "lvl", "Level", "LEVEL"].includes(String(col))
                      return (
                        <td
                          key={col}
                          style={{
