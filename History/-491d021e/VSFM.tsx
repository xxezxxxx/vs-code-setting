...
/** ===== normalizeArgs: props에서 args를 어떤 형태로든 꺼내 표준화 ===== */
function normalizeArgs(props: ComponentProps): RawArgs {
  // 1) 일반적인 케이스: props.args
  const a: any = (props as any).args ?? {};

  // 2) 중첩 케이스: props.args.args
  if (a && typeof a === "object" && a.args && typeof a.args === "object") {
    return a.args as RawArgs;
  }
  // 3) 이미 평평한 케이스
  return a as RawArgs;
}
...
const MyComponent: React.FC<ComponentProps> = (props) => {
  ...
  // 초기 상태 (order 비면 checked, 그래도 없으면 [])
