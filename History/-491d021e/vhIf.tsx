...
const MyComponent: React.FC<ComponentProps> = (props) => {
  ...
  const saveToApi = useCallback(async (payload: ReturnPayload) => {
    ...
    const ctx = { name: payload.name, checked: payload.checked, order: payload.order, options: payload.options, ts: payload.ts };
    ...
    try {
      ...
      const httpOk = (cfg.okCodes ?? [200, 201, 204]).includes(status);
      const bizOk = cfg.successPath ? !!getByPath(data, cfg.successPath) : true;
  ...
  const send = useCallback(async (event: "save" | "close") => {
    ...
    if (event === "save" && args.saveApi?.url) {
      ...
      if (result.ok) {
        ...
        const msg = result.error || `FAIL : (status: ${result.status})`;
        toast.update(id, {
          render: msg,
          type: "error",
          autoClose: 3000,
